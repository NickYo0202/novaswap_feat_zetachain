...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "移除流动性交易构建请求")
public class RemoveLiquidityTransactionRequest {
    ...
    @NotNull(message = "LP代币数量不能为空")
    @Schema(description = "要移除的LP代币数量（Wei单位）", example = "1000000000000000000")
    private BigInteger liquidity;
    
    @Schema(description = "代币A最小返还量（Wei单位），不设置则自动计算")
    private BigInteger amountAMin;
    
    @Schema(description = "代币B最小返还量（Wei单位），不设置则自动计算")
